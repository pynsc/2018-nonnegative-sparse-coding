...
%sparse coding and dimensionality reduction are emerging as a ubiquitous coding strategy  
\revise{the possibility that sparse coding and dimensionality reduction are common coding strategies} across brain regions and modalities. \revise{These principles may allow} neurons to achieve \acf{NSC} by efficiently encoding high-dimensional %stimulus 
\revise{input} spaces using a sparse and parts-based population code.
% \mikeNote{First sentence is still problematic: `ubiquitous', `stimulus spaces', \ldots}
% \emilyNote{better?}
% Reducing the dimensionality of complex, multimodal sensory streams is critically important for \revise{spatially and} \emilyNote{I think we need to make it clearer that metabolic efficiency isn't the only thing we are concerned about, given the common confusion over energy vs statistical efficiency, but I'm not sure of the best way to word this. Maybe change wording from 'metabollically constrained' to a more general 'variety of constraints'?}
% \mikeNote{How about we just cut it? Less is more}
% metabolically constrained brain areas to represent the world.
In this article, we demonstrate that a
variety of neuronal responses
ranging from sensory processing, to spatial navigation, to memory systems
\mikeNote{Do we really talk about memory?}
\emilyNote{No, probably not. While RSC plays a role in episodic memory, the Nitz study wasn't about that. Jeff's writeup of HPC could count, but that's stretching it, probably.}
...
% This finding suggests that \acs{NSC} might be a general principle
...
Brains face the fundamental challenge of processing, storing, and representing high-dimensional stimuli \revise{and input patterns from upstream neurons} using patterns of neural activity. One potential approach to addressing this challenge is to reduce the number of variables required to represent a particular stimulus \revise{or input} space (i.e., dimensionality reduction).
...
% In fact, we propose that \acs{NSC}
% \revise{may be an organizing principle for neuronal computation 
% in many areas of the nervous system.
% \mikeNote{No, we don't}
