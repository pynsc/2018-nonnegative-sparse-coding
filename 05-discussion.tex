...
\revise{We reviewed compelling evidence that a wide range of neuronal responses can be understood as an emergent property of efficient coding due to dimensionality reduction and sparsity constraints.
...
relevant variables, and possibly by the basal ganglia to coordinate movement.}
...
\revise{Analogously, compressed sensing supports dimensionality expansion
by projecting into a larger downstream area \cite{GanguliSompolinsky2012}.}
...
\begin{figure}[ht!]
    ...
    \caption{
        ...
        \emph{Right column}: Location prediction errors.
        The prediction error is based on the how well the neuronal population response can predict the rat's location on the maze. For details, see
        \cite{AlexanderNitz2015,Rounds2018}. 
        Prediction error when comparing even and odd trials
        on the same maze in the same location in the room (prefix $\alpha$ or $\beta$) 
        was much smaller than when the same maze was in different locations (prefix $\alpha\beta$;
        Kruskal-Wallis and Tukey's range test, ***$= p < 0.001$),
        demonstrating that the network can distinguish similar routes
        that occur in different allocentric positions.
        For details see Supplemental Material.}
	}
 	\label{fig:NMF|RSC}
\end{figure}
% \jeffNote{Edited figure 8 caption.  Does this make sense?}         
...
% \jeffNote{I did some restructuring.  I didn't like the "evidence against" subheading.  I think that belongs in Model limitations.  Also didn't like ending with the potential for HPC and PPC. So I broke that out into its own section.  Hope this reads well.}
...
\revise{Furthermore, NSC does not apply to many regions of the brain, especially when the role of the brain area is to cause behavior. Two prominent examples where \ac{NSC} has not been observed are the \ac{PFC} and motor cortex.
...
\cite{Gallego2017,churchland2007,CunninghamYu2014,Kobak2016,Mante2013}.} \revise{In the \ac{PFC}, individual neurons typically encode 
...
% \jeffNote{The following sentence does not follow the previous one. I suggest deleting it. Consequently, many recent studies have adopted dimensionality reduction to analyze these populations and to find features that are not apparent at the level of individual neurons\cite{CunninghamYu2014}}
...
\subsection*{\revise{Potential for NSC in other brain regions}}

\revise{There are several nonsensory areas that may demonstrate \ac{NSC}. In this section we point to evidence that suggests this is the case, but also discuss how sparse activity in these regions differs from \ac{NSC} in sensory systems. We suggest that further studies should be carried out to assess the potential for \ac{NSC} in these regions.}

\revise{\emph{Hippocampus}. There are aspects of the hippocampus that are comparable to the ideas 
...
\revise{\emph{Parietal cortex}.} Analogous to our modeling work in \ac{MSTd} \cite{Beyeler2016},
...
\revise{In conclusion, there is increasing evidence that \ac{NSC} can account for neuronal response properties in a number of sensory and associative cortices, as well as subcortical areas such as the basal ganglia. Although \ac{NSC} might not apply to \ac{PFC} and motor cortex,
