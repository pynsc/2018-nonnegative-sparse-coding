...
\mikeNote{Moved IT to the next para (and addressed your concerns). The face network in IT is complicated, has many different cells (indep. of viewpoint, symmetry, head position, etc.). Very few cells respond to faces as a whole.}
...
% neurons in \ac{IT} \citep{ChangTsao2017},
...
\mikeNote{Even better: The most convincing evidence for our theory comes as part of a just-published Cell article (Chang \& Tsao 2017), now added.}
\citep{ChangTsao2017}.
...
\jeffNote{MC3: 64 means nothing to me unless you can say what it was reduced from. Also, you need to report the sparsity in the MSTd experiments.}
\mikeNote{Now giving the MT numbers and describing the sparsity of the code.}
...
\jeffNote{MC 5: Make the figures as big as possible. Fill the page if you can. We don't want to strain reviewers eyes.}
\jeffNote{fix caption in key figure (see the ???)}
\mikeNote{Also, even in full-page mode the labels in B3 are too small to be readable}
