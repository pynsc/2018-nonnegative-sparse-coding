...
\revise{At the level of neuronal populations,
...
\subsection*{\revise{Sparse coding}}
...
\revise{\emph{Linear sparse coding} is one such strategy,
...
\revise{When Olshausen and Field applied linear sparse coding 
to natural images,
...
\revise{In order to} transform Olshausen and Field's sparse coding
...
\revise{Hoyer \cite{Hoyer2002,Hoyer2003} thus proposed to enforce}
...
\revise{These considerations led to the formulation of \ac{NSC} in its current form.}


\subsection*{\revise{Nonnegative sparse coding (NSC)}}

\revise{As a special case of linear sparse coding,
...
\revise{Consider $S$ observed stimuli or data samples,
...
a vector $\vec{v}_s$ \revise{(i.e., by flattening each observed image)},
...
\end{equation}
where \textbf{W} is a \revise{$F \times B$} matrix that contains as its columns
...
and \textbf{H} \revise{is a $B \times S$ matrix containing} 
...
\revise{In the case of $\lambda = 0$, Eq.~\ref{eqn:nsc-cost-function}
...
\revise{Another open parameter is the number of basis functions, $B$, 
...
the bias (training) error will be reduced, because the model can learn
subtle differences between different contexts with high accuracy.
...
or from spurious correlations:
When a novel context is presented, the model will respond according to
these learned patterns rather than according to the underlying actual relationships,
and the model is said to \emph{overfit} the data,
...
in the bias-variance error trade-off
(labeled `best model' in Fig.~\ref{fig:nsc-bias-variance-dilemma}).}
\mikeNote{We could totally cite our patent here! Should we?}

\begin{figure}[!t]
	\centering
	\includegraphics[width=0.6\textwidth]{fig-rev1-model-complexity}
    \caption{\revise{The bias-variance dilemma.
    With increased model complexity 
...
\revise{Analogously to \cite{OlshausenField1996,OlshausenField1997},
...
corresponds to the number of output neurons, \revise{and the}
response of the $b$-th model output neuron
($b \in [1, ..., B]$)
to a particular input stimulus $s$, \revise{termed} $r_{bs}$,
can be computed by feeding the dot product of the neuron's 
presynaptic \revise{connection weights}
...
by interpreting inhibitory \revise{connection} weights
as nonnegative synaptic conductances.
\revise{This} not only preserves the parts-based quality of the encoding,
...
\revise{connection} weights might not be necessary 
...
\revise{and Eq.~\ref{eqn:nsc-model-response}}
