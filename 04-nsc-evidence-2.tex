...
(Fig.~\ref{fig:NMF|retina}). \revise{This technique, termed \ac{STNMF}, is simply the application of NMF to the activity patterns or stimuli that cause a given neuron to spike. The method was developed by analyzing the spikes produced by retinal ganglion cells when subjected to white noise stimulation, and applying \ac{NMF} to the white noise sequences by which the responses were produced \cite{Liu2017}.}
\emilyNote{summary of STNMF for R1. A schematic of it would probably just confuse them more.}
\mikeNote{Cool!}
...
\emilyNote{addressed part 1 of this comment above. for part 2, they are meant to be the same, right? I understood the modules to be 'neurons' (subunits) in that receptive field - but that must mean the red boxes are subunits too, or that makes no sense. I edited the caption, but please correct if I am wrong.}
...
\begin{figure}[ht]
    ...
    \textbf{\emph{B}},
         ...
         (blue frames), others are more noise-like (red frames). \revise{These modules make up the subunits within a ganglion cell receptive field.}
...
\emilyNote{I will take a stab at this if you don't have time, but I think you can explain it better.}
